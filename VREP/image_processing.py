...
WIDTH_LANE_PIXEL = 180      # Pixels
# Measures
AXIS_X_METERS_PER_PIXEL = 1.0/256
AXIS_Y_METERS_PER_PIXEL = 7.0/256
# Colors
WHITE = 255
BLACK = 0
RED = [255, 0, 0]
...
def add_text_to_image(img, radius_curvature, center):
    ...
    cv.putText(img, curv_text, (15, 25), font, 0.5, (255, 255, 255), 2)

    left_or_right = "esquerda" if center > 0 else "direita"
    cv.putText(img, '%.2fm a %s do centro' % (np.abs(center), left_or_right), (25, 80), font, 0.5,
                (255, 255, 255), 2)
...
def __disconsider_proximity_points(histogram, peak, space_lines=40):
...
def __search_for_peak_histogram(histogram, min_value=1):
...
def fit_lines(binary_img, straight_value=0.0004):
    ...
    half_height = np.int(0.73 * height_img)
    ...
    # Determine the right and left base of lines
        ...
        x_fit = __sliding_window_algorithm(nonzero, first_interval_y, peak_one, bin_img=binary_img)
        x_fit_base_img = __lane_pixels(x_fit, height_img - 1)
        # Histogram from extremity of image
        interval_y = [half_height, np.int(0.9 * height_img)]
        ...
        _, peak_x_left = __search_for_peak_histogram(left_hist, min_value=1)
        _, peak_x_right = __search_for_peak_histogram(right_hist, min_value=1)
        
        print ('peak_x_left', peak_x_left)
        print ('peak_x_right', peak_x_right)
